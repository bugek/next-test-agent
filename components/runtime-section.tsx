type RuntimeSectionState = "loading" | "empty" | "error" | "ready";

type RuntimeSectionItem = {
  label: string;
  value: string;
  detail?: string;
};
...
export function RuntimeSection({ state = "ready", items = [] }: RuntimeSectionProps) {
  if (state === "loading") {
    return (
      <section style={sectionStyles.shell}>
        <p style={sectionStyles.eyebrow}>Editorial product surface</p>
        <h2 style={sectionStyles.title}>Runtime Section</h2>
